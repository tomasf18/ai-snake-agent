...
class Snake:
    ...
    def move(self, mapa, direction: Direction):
        ...
        if new_pos == self.head or new_pos in self._body:
            ...
            logger.debug("Head %s can't move to %s with direction %s", self.head, new_pos, direction)
            self.kill()
            return
...
class Game:
    ...
    def start(self, players_names):
        ...
        self._running = True
        self._snakes = {player_name: Snake(player_name, *self.map.spawn_snake()) for player_name in players_names}
    ...
    def update_snake(self, name):
        try:
            ...
            logger.error(
                "Invalid key <%s> pressed. Valid keys: w,a,s,d", lastkey
            )
    ...
    async def next_frame(self):
        ...
        if self._step % 100 == 0:
            for name, snake in self._snakes.items():
                logger.debug(
                    f"[{self._step}] SCORE {name}: {snake.score}"
                )
